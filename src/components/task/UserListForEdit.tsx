...
interface UserListProps {
    setTeam: (team: User[]) => void; // Update type to accept User objects
    team: User[]; // Array of user objects representing the team
}

const UserListForEdit: React.FC<UserListProps> = ({ setTeam, team }) => {
    ...
    useEffect(() => {
        if (team.length > 0 && data.length > 0) {

            const teamUsers = data.filter(user => team.some(t => t._id === user._id));
...
export default UserListForEdit;
